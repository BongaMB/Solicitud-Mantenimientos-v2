...
function init()
{
    ...
    $(document).on("click", '.edit', function(){ // el document jala todo lo del index, si en el docuemnto en el evento click donde ahi elementos con el evento edit
        var idMod = $(this).attr("data-id_Man");// con el this , jalamos todos los datos y en este caso jalamos ela caja de texto osea el input
        var tipo = $(this).attr("data-Man");
        $('#id_Man').val(idMod);
        $('#Man').val(tipo);
        ...
        $('#Man').focus();//pone el cursor en el nomc
    });

    $(document).on("click", '.delete', function()// es lo mismo que el de "edit" pero ahora con el evento delete
    {
        var id_Man = $(this).attr("data-id_Man");
        var parametros = "id_Man=" + id_Man + "&accion=Eli";
        $.ajax({
            type: "post",
            url: controlador,
            dataType: 'json',
            ...
            success: function(respuesta){
                if (respuesta['status']==1){
                    ...
                    if (id_Man>0){
                        table.row('#' + data.id_Man).remove().draw();
                    }
                    M.toast({html: 'Mantenimiento Eliminado', classes: 'rounded blue lighten-2'});
                }
            } 
        });
    });
...
function validateForm(){ // esto es la validacion del formulario
    $('#formulario').validate({
        rules:{
            Man:{required:true, minlength:1, maxlength:60},
        },
        messages: {
            Man:{required:"No puedes dejar este campo vacío",minlength:"Debes ingresar al menos 4 caracteres", maxlength:"No puedes ingresar más de 60 caracteres"},
        },
        errorElement: "div",
        errorClass: "invalid",
...
function guardarRegistro(){ // este lo guarada
    ...
    if (id_Man > 0){// a qui define si es una insercion o una ctualizacion
    ...
    $.ajax({
        ...
        success: function(respuesta){
            if (respuesta['status']==1){
                ...
                if (id_Man >0){
                    table.row('#' + data.id_Man).remove().draw();//jala todo lo del data en el idclasific
                }
                var row = table.row.add([ //Luego lo vuelve agregar con su respectivo data , que ya trae todo lo del post
                    data.Man,
                    '<i class="material-icons edit" data-id_Man="' + data.id_Man + '" data-Man="' + data.Man +'">create</i><i class="material-icons delete" data-id_Man="' + data.id_Man + '">delete_forever</i>' //boton para eliminar o actualizar dentro de la tabla con sus botones
                ]).draw().node();
                $(row).attr('id',data.id_Man);

                M.toast({html: 'Mantenimiento Guardado', classes: 'rounded blue lighten-2'});
            }
        } 
    });
}
