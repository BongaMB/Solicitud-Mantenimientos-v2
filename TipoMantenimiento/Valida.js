...
function init()
{
    ...
    $(document).on("click", '.edit', function(){ // el document jala todo lo del index, si en el docuemnto en el evento click donde ahi elementos con el evento edit
        var idMod = $(this).attr("data-idTM");// con el this , jalamos todos los datos y en este caso jalamos ela caja de texto osea el input
    ...
    {
        var idTM = $(this).attr("data-idTM");
        var parametros = "idTM=" + idTM + "&accion=Eli";
        $.ajax({
            type: "post",
            url: controlador,
            dataType: 'json',
            ...
            success: function(respuesta){
                if (respuesta['status']==1){
                    ...
                    if (idTM>0){
                        table.row('#' + data.idTM).remove().draw();
                    }
    ...
    $.ajax({
        ...
        success: function(respuesta){
            if (respuesta['status']==1){
                ...
                if (idMod >0){
                    table.row('#' + data.idTM).remove().draw();//jala todo lo del data en el idclasific
                }
                var row = table.row.add([ //Luego lo vuelve agregar con su respectivo data , que ya trae todo lo del post
                    data.tipo,
                    '<i class="material-icons edit" data-idTM="' + data.idTM + '" data-tipo="' + data.tipo +'">create</i><i class="material-icons delete" data-idTM="' + data.idTM + '">delete_forever</i>' //boton para eliminar o actualizar dentro de la tabla con sus botones
                ]).draw().node();
                $(row).attr('id',data.idTM);
